...
const SignUpLastName: React.FC<any> = ({navigation}) => {

    const onboardingContext = useContext(OnboardingContext);

    if (!onboardingContext)
        throw new Error('SignUpLastName must be rendered within an OnboardingContext.Provider');
            ...
            <ScrollView className={"h-screen flex flex-grow mx-5"} stickyHeaderIndices={[0]}>
                ...
                <View className={"mt-10 w-full flex flex-row justify-center items-center"}>
                    <TouchableOpacity className={"flex flex-row items-center justify-center w-48 h-14 rounded-xl bg-[#FC4C02]"} onPress={() => {
                        if(onboardedUser.lastName.length <= 1) {
                            Toast.show({
                                type: 'error',
                                text1: 'Invalid last name',
                                text2: 'Last name should be longer!'
                            });
                            Vibration.vibrate();
                            return;
                        }

                        navigation.push("SignUpCountry")
                    }}>
